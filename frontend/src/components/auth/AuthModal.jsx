...
export default function AuthModal({ isOpen, onClose }) {
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-2xl shadow-xl relative animate-slideUp"
        onClick={(e) => e.stopPropagation()}
        style={{ width: "auto", maxWidth: "95vw" }}
      >
        <button
          onClick={onClose}
          className="absolute top-4 right-4 text-gray-500 hover:text-red-600 transition z-10"
          aria-label="Close"
        >
          <XMarkIcon className="w-6 h-6" />
        </button>
        ...
        {isRegister ? (
  <RegisterForm onSwitch={() => setIsRegister(false)} onClose={onClose} isInModal={true} />
) : (
          <LoginFormContent onClose={onClose} onSwitch={() => setIsRegister(true)} />
        )}
      </div>
